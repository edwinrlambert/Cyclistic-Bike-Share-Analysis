...
create_plot <- function(
    ...
    # Set the base aesthetics for the plots.
    if (plot_type == "pie") {
      base_aes <- aes(x = "", y = !!sym(y_var), 
                      fill = !!sym(fill_var), color = !!sym(fill_var))
    } else {
      base_aes <- aes(x = if (!is.null(x_var)) !!sym(x_var) else NULL, 
                      y = !!sym(y_var), 
                      fill = !!sym(fill_var), color = !!sym(fill_var))
    }
    
    # Create base ggplot objects.
    base_plot <- ggplot(data, base_aes) +
      scale_fill_manual(
    ...
    # Set the geom objects for each plots.
    if (plot_type == "bar") {
      base_plot <- base_plot +
      ...
      base_plot <- base_plot +
        ...
        geom_text(
          aes(label = sprintf("%.1f%%", (!!sym(y_var) / sum(!!sym(y_var))) * 100)),
          position = position_stack(vjust = 0.5),
          color = color_data_point, size = 4) +
        theme_void()
...
rides_by_hour <- cyclistic_data %>%
  group_by(member_casual, hour) %>%
  summarise(total_rides = n(),
            .groups = "drop")

# Create the line chart
ggplot(rides_by_hour, aes(x = hour, y = total_rides, color = member_casual, group = member_casual)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
