...
ggplot(usage_stats, aes(x = "", y = total_rides, fill = member_casual)) +
...
ggplot(usage_stats,aes(x = member_casual, y = (average_ride_length / 60), fill = member_casual)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = sprintf("%.2f", average_ride_length / 60)), 
    position = position_stack(vjust = 0.5),
    color = "white", size = 3.5) +
  scale_fill_manual(values = c("casual" = color_casual, "member" = color_member)) +
  labs(
...
ggplot(usage_stats,aes(x = member_casual, y = average_distance, fill = member_casual)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = sprintf("%.2f", average_distance)), 
    position = position_stack(vjust = 0.5), 
    color = "white", size = 3.5) +
  scale_fill_manual(values = c("casual" = color_casual, "member" = color_member)) +
  labs(
...
day_parts_stats <- cyclistic_data %>% 
...
```{r}
# Extract hour and ensure it's used as a factor for proper grouping
cyclistic_data <- cyclistic_data %>%
  mutate(hour = hour(started_at)) %>%
  mutate(hour = factor(hour, levels = 0:23, labels = paste0(0:23, "h")))  # Label hours as '0h', '1h', ..., '23h'

# Aggregate data by hour and user type
hourly_counts <- cyclistic_data %>%
  group_by(hour, member_casual) %>%
  summarise(total_rides = n(), .groups = "drop")

# Plot the data
ggplot(hourly_counts, aes(x = hour, y = total_rides, fill = member_casual)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  labs(title = "Total Rides by Hour and User Type",
       x = "Hour of the Day",
       y = "Total Rides",
       fill = "User Type") +
  theme_minimal()
